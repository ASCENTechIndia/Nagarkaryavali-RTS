        ...
        const loadPaymentSession = async () => {
            ...
            try {
                const sessionResponse = await axios.get(
                    ...
                    {
                        params: {
                            paymentSessionId: currentTransactionId
                        },
                        headers: {
                ...
                if (!sessionResult?.success || !sessionResult?.data?.length) {
                    throw new Error(
                        ...
                        "Payment session details not found."
                    );
                }
                ...
                await Swal.fire({
                    ...
                    text:
                        ...
                        error?.message ||
                        "Unable to fetch payment session details."
                });
            }
        };
    ...
    useEffect(() => {
        ...
        const processPayment = async () => {
            try {
                ...
                const paymentInsResponse = await axios.post(
                    `${baseUrl}/api/FrmAppFrmAfterTransactionTMCliFee/payment-ins`,
                    {
                        appNo,
                        amount,
                ...
                if (
                    !paymentInsResult?.success ||
                    Number(paymentInsResult?.errorCode) !== 9999
                ) {
                    throw new Error(
                        paymentInsResult?.message ||
                        paymentInsResponse?.data?.message ||
                ...
                if (currentPaymentStatus !== "S") {
                    ...
                    await Swal.fire({
                        ...
                        text:
                            paymentInsResult?.message ||
                            "Bank response indicates that the payment was not successful."
                    });
                    return;
                }
                ...
                Swal.fire({
                    ...
                    text: "Please wait while your receipt and certificate are generated.",
                    allowOutsideClick: false,
                ...
                try {
                    const receiptResponse = await axios.post(
                        `${baseUrl}/api/FrmAfterTransactionTMC/paymentacknowledgement`,
                        {
                    ...
                    const receiptContentType =
                        receiptResponse?.headers?.["content-type"] || "";
                    ...
                    if (!receiptContentType.includes("application/pdf")) {
                        ...
                        const generatedReceiptUrl =
                            window.URL.createObjectURL(receiptBlob);
                ...
                try {
                    const certificateResponse = await axios.post(
                        `${baseUrl}/api/FrmTrackApplication/generate-certificate-report`,
                        {
                            serviceId: String(serviceId),
                            appNo: String(appNo),
                            ulbId: String(ulbId)
                    ...
                    const certificateContentType =
                        certificateResponse?.headers?.["content-type"] || "";

                    if (
                        !certificateContentType.includes(
                            "application/pdf"
                        )
                    ) {
                        const errorText =
                            await certificateResponse.data.text();
