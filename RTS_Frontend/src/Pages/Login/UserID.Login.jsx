...
const SPECIAL_SERVICE_IDS = ["14", "15", "513", "302", "304", "306", "307", "308", "309", "310", "311", "500", "501", "502", "503", "504", "505", "508", "509"];
...
const Login = () => {
    ...
    const handleLogin = async (values) => {
        ...
        try {
            const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/api/auth/login-proc`, loginPayload);
            ...
            if (SPECIAL_SERVICE_IDS.includes(String(serviceId))) {
                const citizenResponse = await axios.post(`${baseUrl}/api/auth/citizen-details`,
                    {mobile: Number(values.mobile.trim())}
                );

                const citizen = citizenResponse.data?.data;

                if (citizen) {
                    const citizenUrl = new URL("https://thanecity.gov.in/tmc/CitizenHome.html");

                    citizenUrl.searchParams.set("loginFlag", "E");
                    citizenUrl.searchParams.set("empname", citizen.mobile);
                    citizenUrl.searchParams.set("password", citizen.password);
                    citizenUrl.searchParams.set("empemail", citizen.email);
                    citizenUrl.searchParams.set("emploginname", citizen.name);

                    window.location.href = citizenUrl.toString();
                    return;
                }
            }
            ...
            setError(err?.response?.data?.message || err?.response?.data?.error  || err?.response?.data?.data?.errorMsg || err?.message || "Login failed");
                    ...
                    <Formik initialValues={{ mobile: "", in_password: "", captcha: "" }} onSubmit={handleLogin}>
                        {({ values, handleChange }) => (
                            <Form className="space-y-4">
                                ...
                                >
                                    {loading ? (
                                        <span className="flex items-center gap-2"><RefreshCWIcon size={16} />Logging in...</span>
                                    ) : (
                                        "Login"
                                    )}
                                </Button>
                                ...
                                {error && (
                                    <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} className="text-center text-sm font-medium text-red-600 underline">
                                        {error}
                                    </motion.div>
                                )}
