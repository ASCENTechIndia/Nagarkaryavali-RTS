...
const SecondAppealPDFHelper = async ({ reportData, filters }) => {
  try {
    if (!reportData.length) throw new Error("No data found.");

    const templatePath = path.resolve(__dirname, "../../templates/FrmSecondAppealAutoRpt.html");
    ...
    const fileName = `Second_Appeal_Report_${Date.now()}.pdf`;
    ...
    console.error("SecondAppealPDFHelper Error:", err);
    throw err;
  }
};

module.exports = { SecondAppealPDFHelper };
