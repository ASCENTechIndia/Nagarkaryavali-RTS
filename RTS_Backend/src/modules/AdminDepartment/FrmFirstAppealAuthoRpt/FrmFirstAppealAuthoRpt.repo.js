...
async function fetchAppealReport({ fromDate, toDate }) {
  const sql = `
    SELECT 
      var_appeal_applicationno AS appno,
      dat_appeal_insdt AS receiptdate,
    ...
    WHERE TRUNC(dat_appeal_insdt) BETWEEN TO_DATE(:fromDate, 'DD-MON-YYYY') AND TO_DATE(:toDate, 'DD-MON-YYYY')
...
module.exports = { fetchAppealReport };
